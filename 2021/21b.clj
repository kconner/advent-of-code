...
(defn take-turn [game whose-turn roll-sum]
  (update game whose-turn
          (fn [[score place]]
       ...
       ((if (<= depth 2) pmap map)
        (fn [[roll-sum frequency]]
          (let [universes (* universes frequency)
                [[score-0 _] [score-1 _] :as game]
                (take-turn game whose-turn roll-sum)]
            (cond (<= 21 score-0) [universes 0]
                  (<= 21 score-1) [0 universes]
                  :else (search universes game
                                (- 1 whose-turn) (inc depth))))))
